...
const ip = 'https://10.20.12.50:8041'

async function get_year(ip, sid) {
    const url = ip + '/pm_api&sid='+ sid;
...
async function get_months(ip, sid, year) {
    const url = ip+'/pm_api&sid='+ sid+ '&year='+year;
    response = await fetch(url);
    if (response.ok) {
        body = await response.text();
        lines = body.split('\n')[0].split(',');
...
async function get_days(ip, sid, month) {
    const url = ip+'/pm_api&sid='+ sid+  '&month=' + month;
    response = await fetch(url);
    if (response.ok) {
        body = await response.text();
        lines = body.split('\n')[0].split(',');
...
async function get_var(ip, sid, date){
  const url = ip+'/pm_api&sid='+ sid + '&date='+ date;
  response = await fetch(url);
  if (response.ok) {
      body= await response.text();
      lines = body.split('\n');
